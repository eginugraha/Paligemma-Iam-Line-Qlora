...
from htr_sp3.english_words import load_english_words  # noqa: E402
...
def main() -> None:
    ...
    p.add_argument(
        "--english-gate", action="store_true",
        help="Option B: widen the validity gate with the vendored English wordlist + treat "
             "possessives/contractions of real words as valid. Off = original train-only gate.",
    )
    args = p.parse_args()
    ...
    # Build the exact-match GATE. The candidate STORE always stays train-only (anti-leakage).
    #   --english-gate OFF: gate = IAM-train words only (the original, pre-Option-B behavior).
    #   --english-gate ON : gate = IAM-train UNION general English wordlist (Option B fix).
    train_records = data.load_iam_splits()["train"]
    if args.english_gate:
        vocab_set = vocab.build_gate_vocabulary(train_records, load_english_words())
    else:
        vocab_set = vocab.build_vocabulary(train_records)
    store = PgVectorStore()  # already populated by ingest_sp3.py

    def make_corrector(threshold: float) -> RagCorrector:
        # possessive_aware tracks the gate widening: it only helps when the gate is wide.
        return RagCorrector(store=store, vocab=vocab_set, threshold=threshold,
                            possessive_aware=args.english_gate)
    ...
    gate = "train+english (Option B)" if args.english_gate else "train-only"
    print(f"[SP-3 tune] gate={gate}  vocab_size={len(vocab_set)}")
